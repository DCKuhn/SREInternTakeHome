...
AWS credentials are not included in this repository. Resources are created in the AWS account associated with the credentials used at runtime. The configuration can be safely destroyed and re-created multiple times. Every push to main on this repo performs this automatically (runs terraform init, terraform plan, terraform apply), via a GitHub Action.
